...
_Welcome screen, account context, costs dashboard (period selector for top tables), recommendations, anomalies, and savings plans._
...
**Web costs dashboard** — The hosted UI includes a structured **costs dashboard** (aligned with AWS Cost Explorer) for the selected AWS profile. A single **Costs period** control sets the window for **both** the **by AWS service** and **by linked account** tables: **month to date** (default) or **last 7 / 30 / 60 / 90 days** (rolling, including today); responses are cached per period for the session. **Marketplace** spend stays month-to-date in this view. You can click rows to drill into **per-account service breakdown** or **per-service account breakdown** (those drill-downs remain **month-to-date** until threaded to the same period in a future change). The same panel surfaces **top cost optimization recommendations**, **Cost Anomaly Detection** anomalies (recent window), and **Savings Plans** utilization and coverage (with optional per-plan detail). A **Cost categories** section (loaded lazily) lists **AWS Cost Categories** rule names in a **summary** table, then **per-rule detail** (value, cost, % of total) in **collapsible** sub-panels—**collapsed by default**—plus a **coverage** summary (categorized vs uncategorized spend) where the API returns data. A **Savings Plans purchase recommendations** section (below cost categories, lazy-loaded) shows **GetSavingsPlansPurchaseRecommendation** results across the supported plan-type / term / payment matrix (payer scope, 30-day lookback). Account identity (account ID, ARN, role) is summarized at the top so you always know which context you are viewing.
...
- **Costs dashboard (web UI)** — By service & by linked account tables with a shared **period** selector (MTD or rolling days); Marketplace and other slices as before; drill-downs; recommendations, anomalies, Savings Plans utilization summaries, cost categories (summary + collapsible per-rule details + coverage), and Savings Plans **purchase** recommendations (CE matrix)
