...
- **Costs dashboard (web UI)** — Month-to-date tables by service, linked account, and Marketplace; drill-downs; recommendations, anomalies, Savings Plans utilization summaries, cost categories (summary + collapsible per-rule details + coverage), and Savings Plans **purchase** recommendations (CE matrix)
