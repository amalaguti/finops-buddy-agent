# cost-categories-dashboard Specification

## Purpose

Hosted costs dashboard support for **AWS Cost Categories**: discover category rule names, show month-to-date spend by dimension value per rule, and coverage (categorized vs uncategorized). Exposes a lazy HTTP slice and demo-safe masking for value labels.
