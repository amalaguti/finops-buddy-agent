# Capability: costs-dashboard-carousel

## Purpose

Define the hosted web UI **carousel presentation mode** for the costs dashboard: one primary block per slide (each `MiniTable` plus the Savings Plans summary card), configurable dwell time, accessibility, and inclusion of all cost category and purchase blocks in the slide sequence.
